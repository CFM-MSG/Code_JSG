...
def compute_query2ctx_info(model, eval_text_dataset, eval_video_dataset, opt, ctx_info):
    ...
    for idx, batch in tqdm(enumerate(query_eval_loader), desc="Computing query2video scores", total=len(query_eval_loader)):
        ...
        # _q2v_props, _q2v_props_scores, _frame_scale_scores = model.get_infer_res(var4ml['video_query'], ctx_info['clip_proposal_feat'], ctx_info['frame_proposal_feat'], num_props_for_each_video) # vcmr on clip+frame branch
