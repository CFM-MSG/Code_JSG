...
def collate_train(data):
    ...
    clip_video_features, frame_video_features, cap_feats, idxs, cap_ids, video_ids = zip(*data)
    ...
    #captions
    feat_dim = cap_feats[0][0].shape[-1]

    merge_cap_feats = []
    all_lengths = []
    labels = []

    for index, caps in enumerate(cap_feats):
        labels.extend(index for i in range(len(caps)))
        all_lengths.extend(len(cap) for cap in caps)
        merge_cap_feats.extend(cap for cap in caps)

    target = torch.zeros(len(all_lengths), max(all_lengths), feat_dim)
    roberta_mask = torch.zeros(len(all_lengths), max(all_lengths))

    for index, cap in enumerate(merge_cap_feats):
