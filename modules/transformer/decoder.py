...
from modules.transformer.mutihead_attention import MultiheadAttention
import pdb
...
class TransformerDecoder(nn.Module):
    ...
    def present_mask(self, tensor):
        if not self.future_mask:
            return None
        dim = tensor.size(0)
        tmp = torch.tril(fill_with_neg_inf(tensor.new(dim, dim)), 1)
        self._present_mask = torch.triu(tmp, 1)
        return self._present_mask[:dim, :dim]
    ...
    def forward(self, src, src_mask, tgt, tgt_mask, src_gauss_weight=None, tgt_gauss_weight=None):
        ...
        for layer in self.decoder_layers:
            x, weight = layer(x, non_pad_tgt_mask,
                              ...
                              # self.buffered_future_mask(x),  # ori: future mask
                              self.present_mask(x), # new: present mask
                              src_gauss_weight, tgt_gauss_weight)
        return x.transpose(0, 1), weight


class TransformerDecoderLayer(nn.Module): # envoder_out=None时相当于encoder
